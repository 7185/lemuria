...
const computeTransition = (firstFrame: any, lastFrame: any, duration: number, frameRate: number) => {
  const transition: ThreeSequence = {original: null, frames: [], frameRate, rootJointTag: 1, keyFrameIDs: []}
  const nbFrames = Math.floor(duration * frameRate)

  transition.frames.push(firstFrame)

  for (let i = 1; i < nbFrames - 1; i++) {
    transition.frames.push({joints: {}, location: new Vector3(0.0, 0.0, 0.0)})
  }
...
export class AvatarAnimationPlayer {
  ...
  constructor(private avatarAnimationManager: AvatarAnimationManager, avatarGroup: Group) {
    this.populateAvatarViewRecursive(avatarGroup)
  }

  public animate(deltaSecond: number, state: string = 'idle', gesture: string = null, velocity: number = 0): boolean {
    // Choose which state to move in based on the velocity

    let newFallbackState = null
    let newState = {name: 'idle', velocityMultiplier: null}
    ...
    if (newGesture !== this.currentGesture && newGesture !== null) {
       ...
       const newSequence = this.avatarAnimationManager.explicitSequences.get(newGesture)
       ...
       if (newSequence) {
         ...
         this.currentTransition = computeTransition(this.lastPlayedFrame, newFrame, transitionDuration,
      ...
      if (newSequence) {
        newFrame.joints = newSequence.frames[0].joints
        newFrame.location.set(0.0, 0.0, 0.0)
      }

      this.currentTransition = computeTransition(this.lastPlayedFrame, newFrame, transitionDuration, this.currentStepState?.frameRate || 60)
      this.currentStepState = null
    }
  ...
  public reset() {
    ...
    for (const entry of Object.entries(this.avatarView)) {
      (entry[1] as Group).setRotationFromQuaternion(new Quaternion(0, 0, 0, 0))
    }

    this.avatarView['1']?.position.set(0, 0, 0)
  }

  public makeNullFrame() {
    const frame = {joints: {}, location: new Vector3()}

    for (const entry of Object.entries(this.avatarView)) {
      frame.joints[entry[0]] = new Quaternion(0, 0, 0, 0)
    }
  ...
  private findBestAnimation(animationList: AnimationEntry[]): any[] {
    ...
    for (const entry of animationList) {
      threeSequence = this.avatarAnimationManager.implicitSequences.get(entry.name)
  ...
  private stepImplicit(animationList: AnimationEntry[], deltaSecond: number, velocity: number = 1) {
    // Iterate over each proposed animation
    const [threeSequence, velocityMultiplier] = this.findBestAnimation(animationList)
    ...
    if (threeSequence) {
      ...
      this.step(threeSequence, velocityMultiplier !== null ? deltaSecond * velocity * velocityMultiplier : deltaSecond,
        false, true)
        return
    }
  ...
  private stepExplicit(animationName: string, frameOffset: number = 0): boolean {
    return this.step(this.avatarAnimationManager.explicitSequences.get(animationName), frameOffset, true, false)
  }

  private step(threeSequence: ThreeSequence, deltaSecond: number, updateLocation: boolean, loop: boolean): boolean {
    if (!this.currentStepState) {
      this.currentStepState = {total: threeSequence.frames.length, frameRate: threeSequence.frameRate, current: 0}
    }

    // Compute the new frame ID, take looping into account
    const current = (this.currentStepState.current + Math.floor(deltaSecond * this.currentStepState.frameRate))
    this.currentStepState.current = loop ? current %
      this.currentStepState.total : current
    ...
    if (this.currentStepState.frameRate !== threeSequence.frameRate) {
      ...
      this.currentStepState.current = Math.floor(this.currentStepState.current * ratio)
    ...
    if (frame === undefined) {
      ...
      if (!updateLocation) {
        this.lastPlayedFrame.location.set(0.0, 0.0, 0.0)
      }
    }
