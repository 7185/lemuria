...
export class AvatarAnimationPlayer {
  ...
  static computeTransition = (firstFrame: Frame, lastFrame: Frame, duration: number, frameRate: number) => {
    ...
    transition.frames.push(...Array.from({length: nbFrames - 1}, () => ({joints: {}, location: new Vector3()})))
  ...
  public animate(deltaSecond: number, state: string = 'idle', gesture: string = null, velocity: number = 0): boolean {
    ...
    if (newGesture !== this.currentGesture && newGesture !== null) {
       ...
       if (newSequence) {
         ...
         this.currentTransition = AvatarAnimationPlayer.computeTransition(this.lastPlayedFrame, newFrame, transitionDuration,
